...
const Dice: React.FC = () => {
  ...
  const handleRoll = () => {
    ...
    const randomY = Math.floor(Math.random() * 4) * 180;

    const newFaceOrder = faceOrder.slice().sort(() => Math.random() - 0.5);

    controls
      .start({
        y: [0, -100, 0], // 주사위가 공중으로 던져지는 높이 조정
        rotateX: [rotation.rotateX, randomX + 360, -30],
        rotateY: [rotation.rotateY, randomY + 360, 30],
        transition: {
          duration: 1,
          ease: 'easeInOut',
        },
      ...
      .then(() => {
        setRotation({ rotateX: -30, rotateY: 30 }); // 초기 상태로 회전 설정
        setFrontFace(newFaceOrder[0]); // 앞면 상태값 업데이트
      });
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-16 h-screen">
      ...
      <div>Current Front Face: {frontFace}</div> {/* 앞면 상태값 표시 */}
    </div>
  );
};
